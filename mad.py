...
Usage:  
    ...
    Flags:
       ...
       -n: Intended for batch processing. Like -m, but pure newick. 
           Only one rooted tree per input tree, errors as a lone ';' in a line.  
           Users should redirect stdout and check for errors and warnings.
           In particular, just one rooted tree means that ties are not
           fully reported!
       ...
       -z: Zero - convert negative branch lengths to 0.0 (for NJ etc.).                
           Default bails out.  
       -b: Bootstrap values - treat internal node labels as branch labels
           and include these in the output newick. Such labels must be numeric. 
           Default is to remove internal node labels from the output newick.  
  -f | -g: Figtree - rooted trees formatted for viewing AD scores in figtree.
     ...
     [ -u: Exclusively for use with pgroot.m. Like -n, but unrooted, flat, and with 
           ADs replacing branch lengths after ':'. Filename ends in '.AD_unrooted'
           and lines begin with blurb to crash unintended usage in other programs. ]                 
       -v: Version.       
       -h: Help.
...
#u: unrooted with ad
#v2.22 11-May-2019
#b: bootstrap values in output
#z: negative lengths to 0
#=====================
#---- init
from sys import argv, stderr, version, exit
from re import sub
fstr="".join([a.upper() for a in argv if a[0]=='-'])
flags={f: fstr.count(f) for f in "MNSPTZBFGVHDU"}
...
if flags['G'] and flags['F']:
    ...
    exit()
if flags['B'] and flags['F']:
    print('Flags -b and -f are mutually exclusive.')
    exit()
if flags['G'] and flags['B']:
    print('Flags -g and -b are mutually exclusive.')
    exit()
if flags['M'] and flags['N']:
    print('Flags -m and -n are mutually exclusive.')
    exit()
if flags['M'] and flags['U']:
    print('Flags -m and -u are mutually exclusive.')
    exit()
if flags['B'] and flags['U']:
    print('Flags -b and -u are mutually exclusive.')
    exit()
...
gdbug_level=flags['D']
at="#^$%"
if flags['M'] or flags['N'] or flags['U']:  err="Error: "
...
if nnwk>1 and not flags['M'] and not flags['N'] and not flags['U']:  exit(err+
...
def nwk2tree(nwkstr):
        ...
        for i in [0,1]:
            ...
            if b[0]!="@":
                ...
                if len(z)!=2:           madlog(err+
                                        "Corrupt NEWICK format: malformed 'node_label:branch_length' clause (...'"+b+"'...).") 
            ...
            if len(ll)!=4:              madlog(err+
                                        "Corrupt NEWICK format: malformed 'node_label:branch_length' clause (...'"+ll[2].replace(at,"@")+"'...).") 
            jnode=int(ll[1])
            labs[jnode]=ll[2]
            if jnode>=notu and len(ll[2])>0:
                try:
                    x=float(ll[2])
                except ValueError:      madlog(err+"Corrupt NEWICK format: invalid bootstrap value (...'"+ll[2]+"'...).")
                bsps[jnode]=ll[2]
            x=str2len(ll[3])
            ...
            if x<0:
                if flags['Z']:
                    x=0
                    madlog("Warning: Negative branch lengths converted to 0")
                else:                   madlog(err+
    ...
    #gdbug(1,[notu,iotu,nnode,inode,tlen,prec,ntiny])
    ...
    return(notu,nnode,rootnode,nodes,blen,trip,labs,prec,tlen,bsps)
#<--nwk2tree()
#========================================
...
def tree2nwk():
    ...
    # ad can be ad+1, see polyroots
    ad1=[x if x<=1 else x-1 for x in ad] 
    if not flags['U']:
        blens=[prec.format(x) for x in blen] 
    else:
        blens=['{:#5.3f}'.format(x) for x in ad1] 
    if flags['F']:
        nads=["[&AD={:#5.3f},ADS={:#5.3f}]:".format(x,x) for x in nad]
    elif flags['G']:
        bads=["[&AD={:#5.3f},ADS={:#5.3f}]:".format(x,x) for x in ad1]
        nads=["[&AD={:#5.3f},ADS={:#5.3f}]:".format(x,x) for x in nad]
        blens1=[prec.format(x) for x in rlen] 
        blens2=[prec.format(x) for x in rlen2] 
    elif flags['B']:
        nads=[x+":" for x in bsps]
    else:
        nads=[":"]*nnode
    clen=0
    while len(nstck)>1:
        ...
        if k<notu:
            ...
            if flags['G']:
                ...
                nwk[k]="({}){}{}".format(nwk[k],nads[k1],blens[k1])
    ...
    tol=(tlen-clen)/tlen
    #gdbug(1,"tol=",tol)
    lnwk=nwk[nodes[rootnode,0]]
    rnwk=nwk[nodes[rootnode,2]]
    newnwk="({},{})".format(lnwk,rnwk)        
...
#========================================
#====== find minimal values, generate AI, reroot
def mad_output():
    global rootnode,nodes,blen,trip,labs,ofn,rlen,rlen2,ad,nad,bsps
    #--------- find mads        
    mad=min(ad)
    ...
    nad[rootnode]=mad
    if nroots>1:
        saved=[deepcopy(v) for v in [labs,blen,nodes,trip,ad,nad,rlen,rlen2,bsps]]
    rooted=""
    ...
    for r in range(nroots):
        ...
        if flags['U']:
            ad[rootnode]=mad
            rlen2[rootnode]=blen[i]+blen[k1]
            if k1<notu:
                blen[k1]=rlen2[rootnode]  
                blen[i]=0           
            else:
                blen[i]=rlen2[rootnode]  
                blen[k1]=0           
        #------- make newick string
        ...
        rooted+="\n"
        if r<nroots-1:
            labs,blen,nodes,trip,ad,nad,rlen,rlen2,bsps=[deepcopy(v) for v in saved]
    #<--r loop
    #--------            
...
"""
    ...
    writeout(rooted)
    #gdbug(1,rooted)
    if flags['U']:  print("Unooted tree with :AD values written to '{}'\n".format(ofn))
    elif nroots>1:  
...
for inwk in range(nnwk):
    if flags['M'] or flags['N'] or flags['U']:
    ...
    try:
        #---
        notu,nnode,rootnode,nodes,blen,trip,labs,prec,tlen,bsps = nwk2tree(nwkstr)
        #---
